...
type Props = {
  ledgerKey: Record<string, string>;
  companyId: string;
};
...
export default function CustomLedger({ ledgerKey, companyId }: Props) {
  ...
  const ledgerId = Object.keys(ledgerKey)[0];
  const ledgerName = Object.values(ledgerKey)[0];
  console.log(ledgerId,ledgerName,companyId,ledgerKey,`${ledgerId} - ${ledgerName}`,`${ledgerId} - ${ledgerName}` === '520102 - Office Supplies');
        ...
        try {
          ...
          const querySnapshot = await getDocs(
            query(
              ...
              where('invoiceData.journalEntry.level4', '==', `${ledgerId} - ${ledgerName}`),
  ...
  const columns: any = [
    ...
    },
    {
      ...
      selector: (row: invoice) => {
        if (row.invoiceData?.journal?.from)
